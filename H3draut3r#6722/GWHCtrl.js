...
let argsSchema = [ // options to add as a flag (--hack % , --useHome, --use_all_purchased , --useNonOwned , --ignore (special ussage) )
  ...
  ['useNonOwned', false], // use non-owned, rooted server as script-server, to enable, run with "--useNonOwned"
...
export async function main(ns) {
  ...
  function upd_ussrvr(ns) { // special function for special use
    let use_servers = []
    if (arg.useAllPurchased || arg.useAll) {
    ...
    for (let tserv of nots()) {
      ...
      for (let scsrv of script_servers) {
        if (doesExist(scsrv.name, ns)) {
          ...
          let sctp = threadPossible(scsrv, swname, ns); // script-servers possible threads for weaken/grow
          // values for hack ⤵
          let sachpr = threadSameArg(scsrv.process_list, shname, tserv.name, ns); // same argument current hostname program running
          let ncsht = Math.ceil(ns.hackAnalyzeThreads(tserv.name, (current_money * hperct))); // needed threads, for X% of targets server money
          let sht = calculateThreads(script_servers, shname, tserv.name); // sum of hack threads on every server with same arg
          let mht = ncsht - sht; // missing hack threads
          let hcsctp = threadPossible(scsrv, shname, ns); // script-servers possible threads for for hack (it's 0.05GB less in size);
          if (arg.debug){console.log(sachpr, timing_array, current_security, min_security, current_money, max_money)}
          ...
          if (!sachpr && ((timing_array.havail == true && (Date.now() >= (timing_array.wstart + timing_array.wtime - timing_array.htime))) || (current_security <= min_security && current_money >= max_money))) {
            ...
            if(arg.debug){console.log(hcsctp, mht)}
            if (hcsctp <= 0 || mht <= 0) {
              await ns.sleep(1)
            }
            else if (hcsctp >= mht) {
          ...
          let mgt = ncgt - sgt;
          if (arg.debug){console.log(sacgpr, mgt)}
          // run grow if need to
          if (!sacgpr && (((timing_array.gavail && (Date.now() >= (timing_array.hstart + timing_array.htime - timing_array.gtime))) || (current_money < max_money && mgt > 0)))) {
            if (arg.debug){console.log(sctp, mgt)}
            if (sctp <= 0 || mgt <= 0) {
              await ns.sleep(1) // 
            }
            else if (sctp >= mgt) {
          ...
          if (arg.debug){console.log(sacwpr,mwt)}
          // run weaken
