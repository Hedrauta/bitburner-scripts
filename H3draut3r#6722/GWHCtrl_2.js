...
let argsSchema = [ // options to add as a flag (--hack % , --useHome, --use_all_purchased , --useNonOwned , --ignore (special ussage) )
  ...
  ['freeHome', 32], // how much free ram "home" will left of (so, at least n amount of RAMs GB will be keep free)
  ['debug', false]
]
...
export async function main(ns) {
  ...
  function upd_ussrvr(ns) { // special function for special use
    ...
    if (arg.useNonOwned || arg.useAll) {
      nors(ns).map(nm => use_servers.push(nm)) // nors is every non-owned rooted server, with ram >= 2GB. function is below
    }
    return use_servers;
  }
  ...
  function nos(ns) {
    ...
    for (var i = 0; i < ns.hacknet.numNodes(); i++) {
      owned_servers.push("hacknet-node-"+i)
    }
  ...
  function nots(ns) {
    return nos(ns).filter(nf => (ns.getServerRequiredHackingLevel(nf) <= player(ns).skills.hacking) && ns.getServerMaxMoney(nf) > 0)
  ...
  let glink = "https://raw.githubusercontent.com/Hedrauta/bitburner-scripts/refs/heads/master/H3draut3r%236722/weaken_grow_ctrl_scripts/grow_server.script";
	let wlink = "https://raw.githubusercontent.com/Hedrauta/bitburner-scripts/refs/heads/master/H3draut3r%236722/weaken_grow_ctrl_scripts/weaken_server.script";
	let hlink = "https://raw.githubusercontent.com/Hedrauta/bitburner-scripts/refs/heads/master/H3draut3r%236722/weaken_grow_ctrl_scripts/hack_server.script";

  await ns.wget(glink, sgname, cur_host);
  await ns.wget(wlink, swname, cur_host);
  await ns.wget(hlink, shname, cur_host);
  async function copy_files() {
    ...
    for (var srvscp of script_servers) {
      if (srvscp != cur_host) { // ignore current server for copy, bc scripts are already existent
				let sfiles = [sgname, shname, swname];
        await ns.scp(sfiles, srvscp.name, cur_host);
				await ns.sleep(1)
      }
    }
  }
  ...
  ns.tprint("Values set on startup:" +
    ...
    "\n".padEnd(4) + "💹🟩".padEnd(5) + "Hacking " + arg.hack + "% of targets Server money.");
  ...
  ns.disableLog("exec");
  ns.disableLog("scp");
  let times = nots(ns).map(am => { return { name: am } });
  // Script-part (in loop)
  ...
  while (true) {
    ...
    let newServers = nots(ns).filter(af => !times.map(tm => tm.name).filter(tf => af == tf))
